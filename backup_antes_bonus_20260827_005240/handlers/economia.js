...
function ganho(ctx,key,cool,min,max,xpMin=15,xpMax=40,texto="Trabalho concluído"){
  ...
  const m=reward(min,max),xp=reward(xpMin,xpMax),old=x.p.level||1;
...
module.exports=async function economia(ctx){
  ...
  if(c==="diario"){
    ...
    const m=reward(500,1000),xp=reward(40,70),old=x.p.level||1;x.p.saldo+=m;x.p.xp+=xp;x.p.level=nivel(x.p.xp);x.p.ultimoDiario=agora;save(x);
  ...
  if(c==="loja"){
    const linhas=Object.entries(loja).map(([id,[nome,preco]])=>`${id}. ${nome} — 🪙 ${dinheiro(preco)}`);
    const cat=(ctx.args[0]||"").toLowerCase();
    let ini=0,fim=70;
    if(cat==="comida"){ini=0;fim=10;}else if(["tecnologia","tech"].includes(cat)){ini=10;fim=20;}else if(cat==="veiculos"){ini=20;fim=30;}else if(cat==="luxo"){ini=30;fim=40;}else if(["miranha","aranha"].includes(cat)){ini=40;fim=70;}
    await ctx.replyText(`🛒 LOJA MIRANHA\n\n${linhas.slice(ini,fim).join("\n")}\n\nUse ${ctx.prefix}comprar ID\nCategorias: comida, tecnologia, veiculos, luxo, miranha`);
    return true;
  }
  ...
  if(c==="comprar"){
    ...
    x.p.saldo-=preco;x.p.inventario[id]=Number(x.p.inventario[id]||0)+1;save(x);await ctx.replyText(`✅ Você comprou ${nome} por 🪙 ${dinheiro(preco)}.`);return true;
  }

  if(c==="inventario"){
    const x=get(ctx.jid,ctx.sender,ctx.metadata);save(x);const itens=Object.entries(x.p.inventario).filter(([,q])=>q>0);
    if(!itens.length)return ctx.replyText("🎒 Seu inventário está vazio."),true;
    await ctx.replyText(`🎒 INVENTÁRIO\n\n${itens.map(([id,q])=>`${loja[id]?.[0]||`Item ${id}`} x${q} — valor un. 🪙 ${dinheiro(loja[id]?.[1]||0)}`).join("\n")}\n\n💰 Saldo: ${dinheiro(x.p.saldo)}`);return true;
  }
