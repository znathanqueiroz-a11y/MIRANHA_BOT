...
async function alvoResgate(sock,jid,metadata,d,tipo,titulo){
  ...
  preparar(d.grupos[jid],tipo,{alvo,liberaEm:Date.now()+180000,liberado:false});
  salvar(d);
...
⌨️ Use !eventoresp A para resgatar ou !eventoresp B para passar.
💰 Resgate: 1.000 a 100.000 moedas.`,
    mentions:[alvo]
  });
...
async function disparar(sock,jid,tipo=null,metadata=null){
  {
    ...
    const _grupo=_estado.grupos?.[jid];
    if(_grupo?.desativado){
      return false; // EVENTOS_DESATIVADOS_GRUPO
    }
  }
...
async function tick(sock){
  if(typeof garantirListener==="function"){
  ...
  for(const jid of Object.keys(inicial.grupos||{})){
    if(!jid.endsWith("@g.us"))continue;

    let atual=estado();
    let g=atual.grupos[jid];

    if(!g)continue;

    if(g.desativado)continue;
    ...
    if(
      ...
      g.ativo.alvo &&
      !g.ativo.liberado &&
      Number(g.ativo.liberaEm||0) > 0 &&
      agora >= Number(g.ativo.liberaEm)
    ){
      g.ativo.liberado=true;
      g.ativo.liberadoEm=agora;
      salvar(atual);

      try{
        await sock.sendMessage(jid,{
          text:`⏰ O tempo da pessoa escolhida acabou!

⚡ Agora vale para TODO MUNDO.

🏆 Quem responder primeiro com:
!eventoresp A

leva a recompensa!`
        });
      }catch(e){
        console.log("⚠️ Liberação do evento:",e.message);
      }
...
if(g.ativo&&Number(g.ativo.expira||0)<=agora){
    ...
    try{
      await disparar(sock,jid);
    }catch(e){
      console.log("⚠️ Animador "+jid+":",e.message);
    }

    // disparar() acabou de salvar "ativo".
    // Recarrega antes de salvar o próximo horário para não apagar o evento.
    atual=estado();
    g=atual.grupos[jid]||(atual.grupos[jid]={});
