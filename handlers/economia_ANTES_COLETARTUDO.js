...
function ganho(ctx,key,cool,min,max,xpMin=15,xpMax=40,texto="Trabalho concluído"){
  const x=get(ctx.jid,ctx.sender,ctx.metadata);
  const agora=Date.now();
  const ultimo=Number(x.p.cooldowns[key]||0);

  if(agora-ultimo<cool){
    return ctx.replyText(
      `⏳ Aguarde ${restante(cool-(agora-ultimo))}.`
    ).then(()=>true);
  }

  const base=reward(min,max);
  const bonus=aplicarBonus(base,x.p);
  const m=bonus.final;

  const xp=reward(xpMin,xpMax);
  const old=x.p.level||1;

  x.p.saldo=Number(x.p.saldo||0)+m;
  x.p.xp=Number(x.p.xp||0)+xp;
  x.p.level=nivel(x.p.xp);
  x.p.cooldowns[key]=agora;

  save(x);

  let bonusTxt="";

  if(Number(bonus.total||0)>0){
    bonusTxt=
      `\n🎁 Bônus da loja: +${bonus.total}%`+
      `\n🪙 Bônus extra: +${dinheiro(bonus.bonusValor)}`;
  }

  return ctx.replyText(
    `${texto}`+
    `\n\n🪙 Ganho base: ${dinheiro(base)}`+
    `${bonusTxt}`+
    `\n💰 Total recebido: ${dinheiro(m)}`+
    `\n✨ +${xp} XP`+
    `${x.p.level>old?`\n🎉 Você subiu para o level ${x.p.level}!`:""}`
  ).then(()=>true);
}
