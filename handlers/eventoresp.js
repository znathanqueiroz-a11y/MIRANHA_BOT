function eventoFinalizadoProtecao(evento){ return Boolean(evento && evento.liberado === true && evento.vencedor); }


function eventoLiberadoFinal(a){
  return Boolean(a && a.liberado === true && !a.eventoConcluido);
}


function bloquearNovoSorteio(evento){
  if(!evento) return false;
  return Boolean(evento.eventoConcluido || evento.vencedor);
}
...
module.exports=async function eventoresp(ctx){
  ...
  const eventoLiberado = a.liberado === true;

  if(a.alvo && !eventoLiberado){
    const idsDoAlvo = idsParticipante(ctx.metadata,a.alvo).map(N);
    const idsDeQuemResponde = idsParticipante(ctx.metadata,ctx.sender).map(N);

    const ehAlvo = idsDeQuemResponde.some(id=>idsDoAlvo.includes(id));

    if(!ehAlvo){
      await ctx.replyText("⏳ A pessoa escolhida ainda tem prioridade neste evento.");
      return true;
    }
  }
  ...
  if(eventoLiberadoFinal(a)){
    a.eventoConcluido = true;
    a.vencedor = ctx.sender;

    await ctx.replyText("🏆 Parabéns! Você foi o primeiro e ganhou a recompensa.");
  ...
  if(a.eventoConcluido){
    await ctx.replyText("⏳ Esse evento já possui vencedor.");
    return true;
  }
