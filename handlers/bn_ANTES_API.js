...
async function pegarUrl(categoria){
  try{
    const j=await jsonFetch(`https://nekos.best/api/v2/${encodeURIComponent(categoria)}`);
    const url=j?.results?.[0]?.url;
    if(url) return url;
  }catch(e){console.log(`⚠️ BN API1 ${categoria}:`,e?.message||e);}

  const fb=WAIFU_FALLBACK[categoria]||"hug";
  const j=await jsonFetch(`https://api.waifu.pics/sfw/${encodeURIComponent(fb)}`);
  if(!j?.url) throw new Error("Nenhuma mídia retornada");
  return j.url;
}
